...
test('a config-model-authored China-shaped compound `when` build-up prices each real branch correctly through engine-core', () => {
  ...
  // Branch 2: direct from a non-LCE supplier -- freight&duty by COO, then 3.2% markup.
  assert.equal(priceFor({ ood: 'SAP', supplier: 'ACME', coo: 'US' }).result.unitPrice, '136.224');
  assert.equal(priceFor({ ood: 'SAP', supplier: 'ACME', coo: 'IT' }).result.unitPrice, '124.872');
  // Branch 3: via LCE/SAP Europe (supplier 88058) -- freight&duty, 3.2% markup, then 6% LCE markup on top.
  assert.equal(priceFor({ ood: 'SAP', supplier: '88058', coo: 'US' }).result.unitPrice, '144.39744');
  assert.equal(priceFor({ ood: 'SAP', supplier: '88058', coo: 'IT' }).result.unitPrice, '132.36432');
});
